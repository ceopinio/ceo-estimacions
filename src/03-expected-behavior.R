...
set.seed(19032024)
grid_partychoice <- grid_partychoice[sample(nrow(grid_partychoice), 250, replace = FALSE),]
...
set.seed(190324)
grid_abstention <- grid_abstention[sample(nrow(grid_abstention), 250, replace = FALSE),]
...
# We fix a really high threshold because we are overestimating participation, to avoid false positives
prob <- 0.94
